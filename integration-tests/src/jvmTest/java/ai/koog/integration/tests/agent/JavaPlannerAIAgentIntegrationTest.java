...
public class JavaPlannerAIAgentIntegrationTest {
    ...
    private static <Plan> void testPlanner(AIAgentPlannerStrategy<String, String, ?> strategy, @Nullable ToolRegistry toolRegistry, String request, String expectedResultPart) {
        var builder = PlannerAIAgent.<String, String>builder(strategy).promptExecutor(promptExecutor).llmModel(OpenAIModels.Chat.GPT4o).systemPrompt(SYSTEM_PROMPT);
    ...
    public void integration_testPlannerWithTools() {
        ...
        var toolRegistry = new ToolRegistryBuilder().tools(new CalculatorTools()).build();
    ...
    public void integration_testGoapPlanner() {
        var formulateAction = Action.<TextualState>builder().name("formulate-problem").precondition(state -> true).belief(state -> new TextualState("Problem: example problem")).execute((context, state) -> {
        ...
        var solveAction = Action.<TextualState>builder().name("solve-problem").precondition(state -> state.text.contains("Problem")).belief(state -> new TextualState("Solution: example solution")).execute((context, state) -> {
            String result = context.llm().writeSession(session -> {
        ...
        var goal = Goal.<TextualState>builder().name("find-solution").cost(state -> 1.0).condition(state -> state.text.contains("Solution")).build();


        var planner = AIAgentPlannerStrategy.builder("custom-goap").goap(TextualState::new).action(formulateAction).action(solveAction).goal(goal).build();
