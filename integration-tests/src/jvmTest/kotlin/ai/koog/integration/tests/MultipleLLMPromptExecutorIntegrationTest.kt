...
class MultipleLLMPromptExecutorIntegrationTest {
    ...
    fun integration_testMarkdownDocument(model: LLModel) = runTest(timeout = 60.seconds) {
        assumeTrue(
            model.capabilities.contains(LLMCapability.Vision.Image),
    ...
    @ParameterizedTest
    @MethodSource("modelClientCombinationsMedia")
    fun integration_testImageFile(model: LLModel) = runTest(timeout = 60.seconds) {
    ...
    @ParameterizedTest
    @MethodSource("modelClientCombinationsMedia")
    fun integration_testTextDocument(model: LLModel) = runTest(timeout = 60.seconds) {
        assumeTrue(
            model.capabilities.contains(LLMCapability.Vision.Image),
            "Model must support image vision capability"
        )
        assumeTrue(
            model.provider != LLMProvider.OpenAI,
            "File format txt not supported for OpenAI"
        )
        ...
        val prompt = prompt("text-document-test") {
            ...
            user {
                markdown {
                    +"I'm sending you a text file. Please repeat its content."
                }
    ...
    @ParameterizedTest
    @MethodSource("modelClientCombinationsMedia")
    fun integration_testPDFDocument(model: LLModel) = runTest(timeout = 60.seconds) {
        assumeTrue(model.capabilities.contains(LLMCapability.Vision.Image), "Model must support vision capability")
    ...
    @ParameterizedTest
    @MethodSource("modelClientCombinationsMedia")
    fun integration_testAudioFile(model: LLModel) = runTest(timeout = 60.seconds) {
