...
Koog follows [Semantic Versioning](https://semver.org/) with the format `X.Y.Z` (e.g., `1.3.0`).
...
Some modules are considered experimental and published with a `-beta` version suffix (e.g., `1.3.0-beta`) rather than the standard `X.Y.Z`. A module may be beta for one of several reasons:
...
| ----------------------- | ------------ | ------------------------------------------------------------------------ |
| `koog-agents`           | `1.3.0`      | All stable modules (transitive) — recommended starting point             |
| `koog-agents-additions` | `1.3.0-beta` | Most beta/experimental modules (except standalone external integrations) |
