...
- `IngestionStorage<DocumentWithMetadata>` — add and update documents
- `RetrievalStorage<DocumentWithMetadata>` — similarity search with filtering and score thresholds
- `FilteringDeletionStorage` — delete by IDs or by filter expression
- `CapabilityAwareStorage` — query supported storage capabilities at runtime
...
> **Note:** Namespace scoping is **not yet implemented**. Passing a non-null `namespace` to any method will throw `IllegalArgumentException`.
...
- Spring AI's `VectorStore` contract exposes similarity search only. Only `SimilaritySearchRequest` is supported; other `SearchRequest` subtypes will throw `IllegalArgumentException`.
...
- Spring AI's `VectorStore` contract has no portable read-by-id API, so `ReadStorage` is intentionally not implemented.
