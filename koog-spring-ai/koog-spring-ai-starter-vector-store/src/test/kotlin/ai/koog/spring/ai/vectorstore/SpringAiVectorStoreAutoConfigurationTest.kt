...
class SpringAiVectorStoreAutoConfigurationTest {
    ...
    fun `should not create Koog storage beans when no VectorStore is present`() {
        contextRunner().run { context ->
            ...
            assertThrows<NoSuchBeanDefinitionException> { context.getBean<IngestionStorage<DocumentWithMetadata>>() }
            assertThrows<NoSuchBeanDefinitionException> { context.getBean<RetrievalStorage<DocumentWithMetadata>>() }
    ...
    fun `should create adapter and Koog storage beans when single VectorStore is present`() {
        contextRunner()
            ...
            .run { context ->
                ...
                assertInstanceOf<SpringAiKoogVectorStore>(context.getBean<IngestionStorage<DocumentWithMetadata>>())
                assertInstanceOf<SpringAiKoogVectorStore>(context.getBean<RetrievalStorage<DocumentWithMetadata>>())
