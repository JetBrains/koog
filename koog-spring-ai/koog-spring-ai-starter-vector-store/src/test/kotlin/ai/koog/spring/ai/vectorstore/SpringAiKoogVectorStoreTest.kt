...
import ai.koog.rag.base.storage.HybridSearchRequest
...
class SpringAiKoogVectorStoreTest {
    ...
    @Test
    fun testCapabilitiesIncludeSimilaritySearchAndFilterExpression() {
        assertEquals(
            setOf(StorageCapability.SIMILARITY_SEARCH, StorageCapability.FILTER_EXPRESSION),
    ...
    @Test
    fun testSearchRejectsHybridSearchRequest() = runTest {
        val ex = assertThrows<IllegalArgumentException> {
            storage.search(HybridSearchRequest(queryText = "hybrid", limit = 1), namespace = null)
        }
        assertTrue(ex.message!!.contains("HybridSearchRequest"))
    }
