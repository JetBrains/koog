...
import ai.koog.rag.base.storage.HasFilterExpression
...
public class SpringAiKoogVectorStore(
    ...
    override val capabilities: Set<StorageCapability> = setOf(
        StorageCapability.SIMILARITY_SEARCH,
        StorageCapability.FILTER_EXPRESSION,
    )
     ...
     */
    override suspend fun search(request: SearchRequest, namespace: String?): List<SearchResult<DocumentWithMetadata>> {
        require(namespace == null) { "Namespace scoping is not yet supported by SpringAiKoogVectorStore" }
        require(request is SimilaritySearchRequest) {
            "SpringAiKoogVectorStore requires a SimilaritySearchRequest, but got ${request::class.simpleName}"
        }
        ...
        return withContext(dispatcher) {
            ...
            val similarityThreshold = (request as? HasScoreThreshold)?.minScore
                ?: SpringAiSearchRequest.SIMILARITY_THRESHOLD_ACCEPT_ALL

            try {
                val filterExpression = (request as? HasFilterExpression)?.filterExpression?.let { text ->
