...
| Apple Foundation Models (iOS 26+, beta)             | [FoundationModelsLLMClient](api:prompt-executor-foundationmodels-client::ai.koog.prompt.executor.clients.foundationmodels.FoundationModelsLLMClient) | -                | -         | -                    | -          | -          | -                                               | On-device iOS-only client backed by `SystemLanguageModel`; single-shot `execute()` with availability gating (`AppleLLMProvider`, `AppleLLModels.SystemDefault`). No streaming, tools, or structured output yet. |
