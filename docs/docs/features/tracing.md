...
=== "Java"
    ...
    import ai.koog.agents.core.agent.AIAgent;
    import ai.koog.agents.features.tracing.feature.Tracing;
    import ai.koog.agents.features.tracing.writer.TraceFeatureMessageFileWriterJvm;
    import ai.koog.agents.features.tracing.writer.TraceFeatureMessageLogWriterJvm;
    ...
    var agent = AIAgent.builder()
        ...
        .install(Tracing.Feature, config -> {
            // Configure message processors to handle trace events
            config.addMessageProcessor(TraceFeatureMessageLogWriterJvm.create(logger));
            config.addMessageProcessor(TraceFeatureMessageFileWriterJvm.create(
                outputPath, 
                path -> { return Files.newOutputStream(path); }
            ));
        })
        .build();
    ```
...
=== "Java"
    ...
    -->
    ```java
    var fileWriter = TraceFeatureMessageFileWriterJvm.create(
        outputPath,
        path -> { return Files.newOutputStream(path); }
    );
...
=== "Java"
    ...
    -->
    ```java
    var writer = TraceFeatureMessageFileWriterJvm.create(
        outputPath,
        path -> { return Files.newOutputStream(path); }
    );
...
=== "Java"
    ...
    -->
    ```java
    var connectionConfig = new DefaultServerConnectionConfig(host, port, null, null);
