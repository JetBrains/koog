package ai.koog.agents.example.utils
...
object Utils {
    object ToneTools {
        ...
        abstract class ToneTool(
            ...
            override suspend fun execute(args: Args): String {
                val executor: PromptExecutor = getMockExecutor {  }
    ...
    object myTool : Tool<myArgs, myResult>(
        argsType = typeToken<myArgs>(),
        resultType = typeToken<myResult>(),
        name = "myToolName",
        description = "myToolDescription"
    ) {
        override suspend fun execute(args: myArgs): myResult = throw NotImplementedError()
    }

    object CreateTool : Tool<CreateTool.Args, String>(
        argsType = typeToken<Args>(),
        resultType = typeToken<String>(),
        name = "message",
        description = "Service tool, used by the agent to talk with user"
    ) {
        data class Args(val s: String)

        override suspend fun execute(args: Args): String = throw NotImplementedError()
    }
    ...
    object SearchTool : Tool<SearchTool.Args, String>(
        argsType = typeToken<Args>(),
        resultType = typeToken<String>(),
        name = "message",
        description = "Service tool, used by the agent to talk with user"
    ) {
        data class Args(val s: String) { fun getQuery(): String = s }
