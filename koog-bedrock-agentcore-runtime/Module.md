...
A Ktor server plugin that implements the [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html)
HTTP contract, so a Koog agent can be deployed and invoked as an AgentCore Runtime workload.

The plugin exposes the two endpoints required by the runtime:
...
- Optional rate limiting for `/invocations` and `/ping`.
- Auto-installs `ContentNegotiation` (JSON) when missing, unless disabled.
