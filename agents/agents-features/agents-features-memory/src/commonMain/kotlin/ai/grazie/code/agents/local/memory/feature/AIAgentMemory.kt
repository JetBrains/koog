 ...
 *
 * @see AIAgentMemoryProvider
 * @see MemoryScopesProfile
 */
class AIAgentMemory(
    internal val agentMemory: AIAgentMemoryProvider,
    ...
    class Config : FeatureConfig() {
         ...
         */
        var memoryProvider: AIAgentMemoryProvider = NoMemory
    ...
    /**
     * Feature companion object that allows installing the [AIAgentMemory] feature in an agent.
     *
     * This object implements [AIAgentFeatureBase] to provide the necessary functionality
     ...
     * ```
     */
    companion object Feature : AIAgentFeatureBase<Config, AIAgentMemory> {
        override val key = createStorageKey<AIAgentMemory>("local-ai-agent-memory-feature")
        ...
        override fun install(config: Config, pipeline: AIAgentPipeline) {
            pipeline.interceptContextStageFeature(this) { stageContext ->
                ...
                AIAgentMemory(config.memoryProvider, stageContext.llm, config.scopesProfile)
            }
        }
    }
 ...
 */
fun AIAgentStageContextBase.memory(): AIAgentMemory = feature(AIAgentMemory.Feature)!!
 ...
 */
suspend fun <T> AIAgentStageContextBase.withMemory(action: suspend AIAgentMemory.() -> T) = memory().action()
