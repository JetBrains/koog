...
public class OpenTelemetry {
    ...
    public companion object Feature : AIAgentGraphFeature<OpenTelemetryConfig, OpenTelemetry> {
        ...
        private fun startCallToolSpan(
            ...
            return if (mcpTool != null) {
                startMcpClientSpan(
                    ...
                    sessionId = mcpTool.sessionId,
                    requestId = null,
                    method = McpMethod.TOOLS_CALL,
                    serverPort = mcpTool.serverPort,
                    serverAddress = mcpTool.serverUrl,
                    mcpProtocolVersion = mcpTool.mcpProtocolVersion,
                    mcpTransportType = mcpTool.mcpTransportType,
                )
            } else {
        ...
        private fun ToolRegistry.getMcpToolMeta(
            toolName: String,
        ): McpServerMeta? = tools.firstNotNullOfOrNull {
            if (it.name == toolName) {
                it.getMeta(McpServerMeta::class)
            } else {
                null
            }
        }
