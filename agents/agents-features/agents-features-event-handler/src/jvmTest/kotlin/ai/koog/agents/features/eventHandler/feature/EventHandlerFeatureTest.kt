...
class EventHandlerTest {
    ...
    fun `test event handler process defined events`() = runBlocking {
        ...
        val expectedEvents = listOf(
            ...
            "OnBeforeLLMWithToolsCall (prompt: [System(content=Test system message), User(content=Test user message, mediaContent=null), Assistant(content=Test assistant response, mediaContent=null, finishReason=null), User(content=Test LLM call prompt, mediaContent=null)], tools: [dummy])",
            "OnAfterLLMWithToolsCall (response: [Default test response], tools: [dummy])",
            "OnAfterNode (node: test LLM call, input: Test LLM call prompt, output: Assistant(content=Default test response, mediaContent=null, finishReason=null))",
            "OnBeforeNode (node: test LLM call with tools, input: Test LLM call with tools prompt)",
            "OnBeforeLLMWithToolsCall (prompt: [System(content=Test system message), User(content=Test user message, mediaContent=null), Assistant(content=Test assistant response, mediaContent=null, finishReason=null), User(content=Test LLM call prompt, mediaContent=null), Assistant(content=Default test response, mediaContent=null, finishReason=null), User(content=Test LLM call with tools prompt, mediaContent=null)], tools: [dummy])",
            "OnAfterLLMWithToolsCall (response: [Default test response], tools: [dummy])",
            "OnAfterNode (node: test LLM call with tools, input: Test LLM call with tools prompt, output: Assistant(content=Default test response, mediaContent=null, finishReason=null))",
