...
class EventHandlerTest {
    ...
    fun `test event handler process defined events`() = runBlocking {
        ...
        val expectedEvents = listOf(
            ...
            "OnBeforeLLMWithToolsCall (prompt: [System(content=Test system message), User(content=Test user message), Assistant(content=Test assistant response), User(content=Test LLM call prompt)], tools: [dummy])",
            ...
            "OnBeforeLLMWithToolsCall (prompt: [System(content=Test system message), User(content=Test user message), Assistant(content=Test assistant response), User(content=Test LLM call prompt), Assistant(content=Default test response), User(content=Test LLM call with tools prompt)], tools: [dummy])",
            "OnAfterLLMWithToolsCall (response: [Default test response], tools: [dummy])",
