...
class GOAPPlannerAgentTest {
    ...
    fun testGOAPLinearPath() = runTest {
        val strategy = goap("goap-linear-test", { _: Unit -> SimpleState() }) {
            action(
                name = "Get key",
    ...
    fun testGOAPOptimalPathSelection() = runTest {
        val strategy = goap("goap-optimal-path-test", { _: Unit -> PathState() }) {
    ...
    fun testGOAPComplexDependencies() = runTest {
        val strategy = goap("goap-complex-test", { _: Unit -> ComplexState() }) {
