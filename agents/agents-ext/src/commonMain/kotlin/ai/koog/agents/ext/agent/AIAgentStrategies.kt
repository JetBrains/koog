 ...
 * @param execute A suspendable function that accepts the input of type `Input` and produces a string output
...
public inline fun <reified Input, reified Output> structuredOutputWithToolsStrategy(
    ...
    noinline execute: suspend AIAgentGraphContextBase.(input: Input) -> String
    ...
    val input by node<Input, String> { input ->
        execute(input)
    }
    val nodeCallLLMMultiple by nodeLLMRequestMultiple()
    val nodeExecuteTools by nodeExecuteMultipleTools(parallelTools = parallelTools)
    val nodeSendToolResult by nodeLLMSendMultipleToolResults()

    val nodeTransformStructuredOutput by node<Message.Assistant, Output> { response ->
        llm.writeSession {
    ...
    nodeStart then setStructuredOutput then input then nodeCallLLMMultiple

    // On tools
    edge(nodeCallLLMMultiple forwardTo nodeExecuteTools onMultipleToolCalls { true })
    edge(nodeExecuteTools forwardTo nodeSendToolResult)

    // On assistant messages
    edge(
        nodeCallLLMMultiple forwardTo nodeTransformStructuredOutput
            onMultipleAssistantMessages { true }
            transformed { it.single() }
    )

    // Post tool result
    edge(nodeSendToolResult forwardTo nodeExecuteTools onMultipleToolCalls { true })
    edge(
        nodeSendToolResult forwardTo nodeTransformStructuredOutput
            onMultipleAssistantMessages { true }
    ...
    // Finish
    edge(
        nodeTransformStructuredOutput forwardTo nodeFinish
    )
}
