...
**Transitions**: Define the execution flow between agents, optionally with conditions based on agent outputs. Agents without explicit outgoing transitions are automatically connected to the finish node.

**Finish Node (`__finish__`)**: A special reserved node name that marks the end of flow execution. All agents without explicit outgoing transitions automatically connect to this node.
...
4. **Output Phase**: Returns final `FlowDataType` result

### Key Design Patterns

- **Configuration-as-Code**: Workflows defined declaratively in JSON, executed imperatively by Koog
- **Type-Safe Inputs/Outputs**: All agent inputs/outputs use sealed `FlowDataType` hierarchy for compile-time safety
...
```kotlin
sealed interface FlowDataType {
    sealed interface FlowPrimitiveType : FlowDataType  // For condition evaluation

    // Primitives
    data class FlowInteger(val data: Int) : FlowPrimitiveType
    data class FlowDouble(val data: Double) : FlowPrimitiveType
    data class FlowString(val data: String) : FlowPrimitiveType
    data class FlowBoolean(val data: Boolean) : FlowPrimitiveType

    // Arrays
    data class FlowArrayInteger(val data: Array<Int>)
    data class FlowArrayDouble(val data: Array<Double>)
    data class FlowArrayString(val data: Array<String>)
    data class FlowArrayBoolean(val data: Array<Boolean>)

    // Special (verify agent output)
    data class FlowCritiqueResult(
        val success: Boolean,
        val feedback: String,
...
- The special `"__finish__"` target marks the end of flow execution.
- **Automatic Termination**: Agents without any outgoing transitions in the configuration are automatically connected to `__finish__`. You don't need to explicitly define transitions to `__finish__` for terminal agents.

**Example - Implicit Finish**:
```json
{
  "agents": [
    {"name": "agent1", "type": "task", "params": {"task": "Do something"}},
    {"name": "agent2", "type": "task", "params": {"task": "Do final step"}}
  ],
  "transitions": [
...
```
In this example, `agent2` automatically connects to `__finish__` since it has no outgoing transitions defined.
  ...
  "transitions": [
    {
      "from": "verify_agent",
      "to": "__finish__",
      "condition": {"variable": "output.success", "operation": "equals", "value": true}
    }
  ]
}
```
Here, the flow explicitly terminates when the verification succeeds.
...
3. Execute flow with input
   ...
   │   - Automatically connects agents without outgoing transitions to __finish__
   │
   ├── Execute graph
   │   - Starts at first agent
   │   - Evaluates conditions on transitions to route to next agent
   │   - Continues until reaching __finish__ node or error occurs
   │
   └── Return final output from __finish__ node
```

## Keywords and Variable Access

The protocol uses two keywords for accessing data in different contexts:

### `output.` - Condition Variables (Transitions)

Used in **transition conditions** to access properties from the **previous agent's output**.

**Format**: `"output.<property>"`

**Examples**:
- `"output.data"` - Access the data field from primitive types (`FlowString`, `FlowInteger`, etc.)
- `"output.success"` - Access the success field from `FlowCritiqueResult`
- `"output.feedback"` - Access the feedback field from `FlowCritiqueResult`
...
### `input.` - Transformation Values (Transform Agents)

Used in **transform agent parameters** to extract fields from the **incoming input** for the current agent.

**Format**: `"input.<field>"`

**Examples**:
- `"input.success"` - Extract the success field from `FlowCritiqueResult` input
- `"input.feedback"` - Extract the feedback field from `FlowCritiqueResult` input
...
**Key Distinction**:
- **`output.`** = "What did the previous agent produce?" (used in conditions to decide routing)
- **`input.`** = "What field should I extract from my input?" (used in transformations to reshape data)

## Condition Evaluation

Conditions in transitions allow routing based on agent outputs:
- Extract values from `FlowDataType` using paths like `"output.success"` or `"output.data"`
...
val input = FlowDataType.FlowString("Your task description here")
val result: FlowDataType = flow.run(input)
```
...
- Verify agent validates (returns `FlowCritiqueResult`)
...
|-----------------|-----------------------|------------------------------------------------|----------------------------------------------------------------------------|
| `EQUALS`        | Exact value match     | Route based on status, type, or boolean flag   | `{"variable": "output.data", "operation": "EQUALS", "value": "invoice"}`  |
| `NOT_EQUALS`    | Value mismatch        | Exclude specific values or types               | `{"variable": "output.data", "operation": "NOT_EQUALS", "value": "en"}`   |
| `MORE`          | Greater than          | Route high priority items                      | `{"variable": "output.data", "operation": "MORE", "value": 80}`           |
| `LESS`          | Less than             | Filter low scores or values                    | `{"variable": "output.data", "operation": "LESS", "value": 50}`           |
| `MORE_OR_EQUAL` | Greater than or equal | Threshold-based routing (≥ 80 = high)          | `{"variable": "output.data", "operation": "MORE_OR_EQUAL", "value": 80}`  |
| `LESS_OR_EQUAL` | Less than or equal    | Maximum value filtering                        | `{"variable": "output.data", "operation": "LESS_OR_EQUAL", "value": 100}` |
| `NOT`           | Boolean negation      | Invert boolean conditions                      | `{"variable": "output.data", "operation": "NOT", "value": true}`          |
| `AND`           | Logical AND           | Combine multiple boolean conditions            | `{"variable": "output.data", "operation": "AND", "value": true}`          |
| `OR`            | Logical OR            | Alternative boolean conditions                 | `{"variable": "output.data", "operation": "OR", "value": false}`          |

### Condition Evaluation Notes

1. **Type Compatibility**: For `EQUALS`/`NOT_EQUALS`, types must match exactly (e.g., `FlowInteger(42)` ≠ `FlowDouble(42.0)`)
2. **Numeric Comparisons**: `MORE`, `LESS`, `MORE_OR_EQUAL`, `LESS_OR_EQUAL` work with `FlowInteger` and `FlowDouble`
3. **String Comparisons**: String operations use case-insensitive lexicographic comparison
4. **FlowCritiqueResult**: Access fields via `output.success` or `output.feedback` in condition variables
5. **Boolean Operations**: `NOT`, `AND`, `OR` only work with boolean values
6. **Variable Prefix**: All condition variables must use the `output.` prefix to access the previous agent's output
...
**Key Features**: `FlowCritiqueResult`, transform agent for feedback extraction, loop until success.
...
val result = flow.run(FlowDataType.FlowString("Test input"))
```
...
fun testConditionalFlow() {
    ...
    val flow = KoogFlow(/* ... */)
    val result = flow.run(FlowDataType.FlowInteger(85))

    // Assert expected routing and output
    assertTrue(result is FlowDataType.FlowString)
    assertContains((result as FlowDataType.FlowString).data, "high score")
}
```
