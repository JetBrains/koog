...
class ToolRegistryTest {
        ...
        assertEquals(
            expected = mapOf(
                stage1 to listOf(
                    tool1,
                    tool2,
                    CollectToolsForStageTool(stage1ToolListName, listOf(tool1, tool2).map { it.descriptor })
                ).map { it.descriptor },

                stage2 to listOf(
                    tool3,
                    CollectToolsForStageTool(stage2ToolListName, listOf(tool3.descriptor))
                ).map { it.descriptor}
            ),
    ...
    fun testSimpleToolRegistryCreatesDefaultStage() {
        ...
        assertEquals(true, simpleRegistry.stagesToolDescriptors.containsKey(StageTool.DEFAULT_STAGE_NAME))

        // Verify that the stage can be retrieved by name
        val stage = simpleRegistry.getStageByName(StageTool.DEFAULT_STAGE_NAME)
        assertEquals(StageTool.DEFAULT_STAGE_NAME, stage.name)
    }
    ...
    fun testSimpleToolRegistryCreatesFromStage() {
        ...
        // Verify that the stage can be retrieved by name
        val stage1 = simpleRegistry.getStageByName(StageTool.DEFAULT_STAGE_NAME)
        assertEquals(StageTool.DEFAULT_STAGE_NAME, stage1.name)
        ...
        assertEquals(true, simpleRegistry2.stagesToolDescriptors.containsKey(StageTool.DEFAULT_STAGE_NAME))

        // Verify that the stage can be retrieved by name
        val stage2 = simpleRegistry2.getStageByName(StageTool.DEFAULT_STAGE_NAME)
        assertEquals(StageTool.DEFAULT_STAGE_NAME, stage2.name)
    }
    ...
    fun testSimpleToolRegistryRegistersTools() {
        ...
        assertEquals(3, simpleRegistry.stagesToolDescriptors[StageTool.DEFAULT_STAGE_NAME]!!.size)

        // Verify that the tools can be found by name
        assertEquals(StageTool.DEFAULT_STAGE_NAME, simpleRegistry.getStageByTool(simpleTool1.name).name)
        assertEquals(StageTool.DEFAULT_STAGE_NAME, simpleRegistry.getStageByTool(simpleTool2.name).name)
    }
    ...
    fun testCombineSimpleToolRegistryWithOtherRegistry() {
        ...
        assertEquals(true, combinedRegistry.stagesToolDescriptors.containsKey(StageTool.DEFAULT_STAGE_NAME))
        ...
        assertEquals(StageTool.DEFAULT_STAGE_NAME, combinedRegistry.getStageByTool(simpleTool.name).name)
        assertEquals(stage1, combinedRegistry.getStageByTool(tool1.name).name)
        assertEquals(stage2, combinedRegistry.getStageByTool(tool3.name).name)
    }
