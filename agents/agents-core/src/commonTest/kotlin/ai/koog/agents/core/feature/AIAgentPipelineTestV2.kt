...
class AIAgentPipelineTestV2 {
    ...
    fun `test pipeline interceptors for node events`() = runTest {
        ...
        val strategy = strategy("test-interceptors-strategy") {
            ...
            nodeStart forwardTo dummyNode transformed { }
            dummyNode forwardTo nodeFinish transformed { "Done" }
        }
    ...
    fun `test pipeline interceptors for llm call events`() = runTest {
        ...
        val strategy = strategy("test-interceptors-strategy") {
            ...
            nodeStart forwardTo llmCallWithoutTools transformed { "Test LLM call prompt" }
            llmCallWithoutTools forwardTo llmCall transformed { "Test LLM call with tools prompt" }
            llmCall forwardTo nodeFinish transformed { "Done" }
        }
    ...
    fun `test pipeline interceptors for tool call events`() = runTest {
        ...
        val strategy = strategy("test-interceptors-strategy") {
            ...
            nodeSendInput forwardTo toolCallNode onToolCall { true }
            toolCallNode forwardTo nodeFinish transformed { it.content }
        }
    ...
    fun `test pipeline interceptors before agent started events`() = runTest {
        ...
        val strategy = strategy("test-interceptors-strategy") {
            nodeStart forwardTo nodeFinish transformed { "Done" }
        }
