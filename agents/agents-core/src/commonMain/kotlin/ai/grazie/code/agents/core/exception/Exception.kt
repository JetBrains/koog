...
/**
 * Base class for all exceptions thrown by `IdeFormer`.
 */
sealed class AIAgentException(message: String) : RuntimeException(message)
 ...
 */
class ToolNotRegisteredException(name: String) : AIAgentException("Tool not registered: \"$name\"")

/**
 * Base class for representing an [ai.grazie.code.agents.core.model.AIAgentServiceError] response from the server.
 */
sealed class AIAgentEngineException(message: String) : AIAgentException(message)
...
class AgentNotFoundException(message: String) : AIAgentEngineException(message)
