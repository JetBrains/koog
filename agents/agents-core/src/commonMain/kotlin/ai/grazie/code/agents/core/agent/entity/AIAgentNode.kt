 ...
 */
abstract class AIAgentNodeBase<Input, Output> internal constructor() {
     ...
     */
    var edges: List<AIAgentEdge<Output, *>> = emptyList()
        private set
    ...
    @InternalAgentsApi
    suspend fun resolveEdgeUnsafe(context: AIAgentStageContextBase, nodeOutput: Any?) =
     ...
     */
    abstract suspend fun execute(context: AIAgentStageContextBase, input: Input): Output
    ...
    @Suppress("UNCHECKED_CAST")
    @InternalAgentsApi
    suspend fun executeUnsafe(context: AIAgentStageContextBase, input: Any?): Any? {
 ...
 */
class AIAgentNode<Input, Output> internal constructor(
...
open class StartAIAgentNodeBase<Input>() : AIAgentNodeBase<Input, Input>() {
    var subgraphName: String? = null
        internal set

    override val name: String get() = subgraphName?.let { "__start__$it" } ?: "__start__"

    override suspend fun execute(context: AIAgentStageContextBase, input: Input) = input
}


open class FinishAIAgentNodeBase<Output>() : AIAgentNodeBase<Output, Output>() {
    var subgraphName: String? = null
        internal set
    ...
    override suspend fun execute(context: AIAgentStageContextBase, input: Output) = input
}

class StartAIAgentNode internal constructor() : StartAIAgentNodeBase<Unit>()

class FinishAIAgentNode internal constructor() : FinishAIAgentNodeBase<String>()