...
public object RetroLLMFit {
    ...
    internal fun <Req : Any, Res : Any> createInternal(
        ...
        for (param in ctor.parameters) {
            when {
                ...
                param.hasAnnotation<MessagesField>() -> {
                    val fmt = param.findAnnotation<MessagesField>()!!.format
                    params += param to ParamKind.Messages(fmt)
                }
                param.hasAnnotation<ToolsField>() -> {
                    val fmt = param.findAnnotation<ToolsField>()!!.format
                    ...
                    when {
                        ...
                        prop?.hasJvmAnnotation(MessagesField::class.java) == true -> {
                            val fmt = prop.javaField!!.getAnnotation(MessagesField::class.java)!!.format
                            params += param to ParamKind.Messages(fmt)
                        }
                        ...
                        prop?.hasJvmAnnotation(ToolsField::class.java) == true -> {
                            val fmt = prop.javaField!!.getAnnotation(ToolsField::class.java)!!.format
                            params += param to ParamKind.Tools(fmt)
                        }
        ...
        val responseFinishReason = resClass.findAnnotation<ResponseFinishReason>()?.path
        ...
        val resMeta = ResponseMeta(
            textProperty = textProperty as kotlin.reflect.KProperty1<Res, *>?,
            textPath = responseText,
            finishReasonPath = responseFinishReason,
