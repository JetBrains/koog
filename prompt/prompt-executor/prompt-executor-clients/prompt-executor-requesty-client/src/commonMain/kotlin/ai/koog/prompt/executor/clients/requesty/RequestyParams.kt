...
internal fun LLMParams.toRequestyParams(): RequestyParams {
    if (this is RequestyParams) return this
    return RequestyParams(
...
/**
 * Requesty chat-completions parameters layered on top of [LLMParams].
 *
 * Requesty exposes the OpenAI Chat Completions format, so the supported knobs match the OpenAI ones.
 *
 * @property temperature Sampling temperature in [0.0, 2.0]. Higher means more random,
 *   lower means more deterministic. Adjust this **or** [topP], not both.
 ...
 * @property speculation Predicted output content used for speculative decoding on models that support it.
 ...
 * @property user A unique identifier representing the end-user, forwarded to the underlying provider.
 * @property additionalProperties Additional properties that can be used to store custom parameters.
 * @property frequencyPenalty Number in [-2.0, 2.0] that penalizes frequent tokens to reduce repetition.
 * @property presencePenalty Number in [-2.0, 2.0] that encourages introduction of new tokens/topics.
 * @property logprobs Whether to include log-probabilities for output tokens.
 * @property stop Stop sequences (1 to 4 items); generation halts before any of these.
 * @property topLogprobs Number of top alternatives per position (0 to 20). Requires [logprobs] = true.
 * @property topP Nucleus sampling in [0.0, 1.0]; use **instead of** [temperature].
 */
@Suppress("LongParameterList")
public class RequestyParams(
