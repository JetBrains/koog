...
class OllamaThinkingFeatureTest {
    ...
    fun `test streaming response with thinking content`() = runTest {
        ...
        val thinkingFrames = streamFrames.filterIsInstance<StreamFrame.AppendThinking>()
        assertTrue(thinkingFrames.isNotEmpty(), "Should have at least one thinking frame")

        // Verify thinking content is present
        val allThinkingContent = thinkingFrames.joinToString("") { it.text }
    ...
    fun `test streaming response with content only`() = runTest {
        ...
        val appendFrames = streamFrames.filterIsInstance<StreamFrame.Append>()
        assertTrue(appendFrames.isNotEmpty(), "Should have at least one append frame")

        // Verify content is present
        val allContent = appendFrames.joinToString("") { it.text }
    ...
    fun `test streaming response with both thinking and content`() = runTest {
        ...
        assertTrue(appendFrames.isNotEmpty(), "Should have append frames")

        // Verify content
        val allThinking = thinkingFrames.joinToString("") { it.text }
        val allContent = appendFrames.joinToString("") { it.text }

        assertTrue(allThinking.contains(thinkingContent), "Thinking content should be in thinking frames")
        assertTrue(allContent.contains(responseContent), "Response content should be in append frames")
    }
    ...
    fun `test token counts are properly extracted with thinking`() = runTest {
        ...
        assertEquals(promptTokens, response.metaInfo.inputTokensCount)
        assertEquals(responseTokens, response.metaInfo.outputTokensCount)
    }
}
