...
public interface PromptExecutorAPI : AutoCloseable {
     ...
     * @param hook Optional executor hook invoked around model resolution and execution lifecycle events.
     ...
     * @param tools An optional list of `ToolDescriptor` objects that define the tools available for the execution.
     * @param hook Optional executor hook invoked around model resolution and streaming lifecycle events.
    ...
    public suspend fun executeMultipleChoices(
        ...
        listOf(
            execute(
                ...
                model,
                tools,
                hook = hook?.adaptResult { responses: List<Message.Response> -> listOf(responses) }
            )
        )
     ...
     * @param hook Optional executor hook invoked around model resolution and moderation lifecycle events.
